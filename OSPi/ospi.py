...
def baseurl():
	...
	#Example baseurl -> 192.168.1.232:8080"""
...
def clear_mm():
    ...
    if gv.sd['mm']:
        ...
        gv.rs = []
        for i in range(gv.sd['nst']):
            gv.rs.append([0,0,0,0]) #added another variable for enable and disable
...
def update_greeting():
    """Updates greeting that is displayed on the user interface"""
    gv.newnow = datetime.datetime.now()

    gv.month = ((gv.newnow).month)
    gv.day = ((gv.newnow).day)
    gv.dayofweek = ((gv.newnow).weekday()) #as int monday -> 0 & sunday -> 6
    gv.hour = ((gv.newnow).hour)-5
    gv.minute = ((gv.newnow).minute)
    
    gv.bdaystring = str(gv.sd['bday'])
    gv.bdaymonth = int(gv.bdaystring[:2])
    gv.bdayday = int(gv.bdaystring[3:5])
    
    gv.greeting = ""
	
    if(gv.month == gv.bdaymonth and gv.day == gv.bdayday):
    	gv.greeting = "Happy Birthday"
	
    if(gv.greeting == ""):
    	if(gv.month == 1):
    	#January Holidays
            if(gv.day==1):
	       	gv.greeting = "Happy New Year"
	elif(gv.month == 2):
	#February Holidays
	    if(gv.day==2):
		gv.greeting = "Happy Groundhog Day"
            elif(gv.day==14):
		gv.greeting = "Happy Valentines Day"
	elif(gv.month==3):
	#March Holidays
            if(gv.day==14):
		gv.greeting = "Happy Pi Day"
	    elif(gv.day==17):
		gv.greeting = "Happy St. Patrick's Day"
	elif(gv.month == 4):
	#April Holidays
	    if(gv.day==1):
		gv.greeting = "Happy April 2nd"
	elif(gv.month == 5):
	#May Holidays
	    if(gv.day==1):
		gv.greeting = "Happy May Day"
	    elif(gv.day>22 and gv.dayofweek==0):
		gv.greeting = "Happy Memorial Day"
	elif(gv.month == 6):
	#June Holidays
	    if(gv.day==14):
		gv.greeting = "Happy Flag Day"
	elif(gv.month == 7):
	#July Holidays
	    if(gv.day==4):
		gv.greeting = "Happy 4th of July"
	#elif(gv.month == 8):
	#August Holidays
	elif(gv.month == 9):
	#September Holidays
	    if(gv.day<6 and gv.dayofweek==0):
		gv.greeting = "Happy Labor Day"
	elif(gv.month == 10):
	#October Holidays
	    if(gv.day==31):
		gv.greeting = "Happy Halloween"
	elif(gv.month == 11):
	#November Holidays
	    if(gv.day==11):
		gv.greeting = "Happy Veterans Day"
	    elif(gv.day>22 and gv.dayofweek==3):
		gv.greeting = "Happy Thanksgiving"
	elif(gv.month == 12):
	#December Holidays
	    if(gv.day==24):
		gv.greeting = "Merry Christmas Eve"
	    elif(gv.day==25):
		gv.greeting = "Merry Christmas"
	    elif(gv.day==31):
		gv.greeting = "Happy New Year's Eve"
	
    if(gv.greeting==""):
	if(gv.hour < 12):
	    gv.greeting = "Good Morning"
	elif(gv.hour < 18):
	    gv.greeting = "Good Afternoon"
	elif(gv.hour < 24):
	    gv.greeting = "Good Evening"
	else:
	    gv.greeting = "Hello"
                ...
                if ((gv.rs[sid][4])==0):
                    ...
                    if gv.rs[sid][2]: # if station has a duration value
                        if not rain or gv.sd['ir'][b]&1<<s: # if no rain or station ignores rain
                            ...
                            accumulate_time = (accumulate_time) - (gv.rs[sid][2])
                        else:
...
def data(dataf):
    ...
    try:
        ...
        if dataf == 'snames': ## A config file -- return defaults and create file if not found. ##
            data = "['Valve 1','Valve 2','Valve 3','Valve 4','Valve 5','Valve 6','Valve 7','Valve 8',]"
...
gv.sd = ({u"en": 1, u"seq": 1, u"mnp": 32, u"ir": [0], u"rsn": 0, u"htp": 8080, u"nst": 8,
            ...
            u"urs": 0, u"nopts": 27, u"pwd": u"b3BlbmRvb3I=", u"password": u"", u"salt": u"", u"ipas": 0, u"rst": 1,
            u"mm": 0, u"mo": [0], u"rbt": 0, u"mtoff": 0, u"nprogs": 1, u"nbrd": 1, u"sh":1, u"fname":"First", u"lname":"Last", u"bday":"06/17/93", u"email":"", u"ltct":32, u"lwlt":60, u"hwlt":200, u"tu": u"C",
...
gv.rs = [] #run schedule
for i in range(gv.sd['nst']):
    gv.rs.append([0,0,0,0]) #scheduled start time, scheduled stop time, duration, program index, enable/disable
...
class change_options:
    ...
    def GET(self):
        ...
        #new options
        if qdict.has_key('ofname'):
            gv.sd['fname'] = qdict['ofname']
        if qdict.has_key('olname'):
            gv.sd['lname'] = qdict['olname']
        if qdict.has_key('obday'):
            gv.sd['bday'] = qdict['obday']
        if qdict.has_key('oemail'):
            gv.sd['email'] = qdict['oemail']

        if qdict.has_key('oltct'):
            gv.sd['ltct'] = qdict['oltct']
        if qdict.has_key('olwlt'):
            gv.sd['lwlt'] = qdict['olwlt']
        if qdict.has_key('ohwlt'):
            gv.sd['hwlt'] = qdict['ohwlt']
        ...
        #start of new code needed a way to run update_shcount
        if int(qdict['osh']) != gv.sd['sh']: self.update_shcount(qdict)
        gv.sd['sh'] = int(qdict['osh'])+1
        #end of new code
        ...
        if qdict.has_key('osh'):
            gv.sd['sh']= int(qdict['osh'])
    ...
    def update_scount(self, qdict):
        ...
        if int(qdict['onbrd'])+1 > gv.sd['nbrd']: # Lengthen lists
            ...
            for i in range((incr*8)+1):
                nlst.append("'Valve "+('%d'%(i+ln))+"'")
            nstr = '['+','.join(nlst)
            nstr = nstr.replace("', ", "',")+"]"
            save('snames', nstr)
            #new function adding hubnames to update when scount is updated (referencing a long list from a short list - index error)
            hubnames = data('hubnames')
            hubnames = ast.literal_eval(hubnames)
            for i in range(0, gv.sd['sh']):
                for j in range(0, ((((int(qdict['onbrd']))+1)-(gv.sd['nbrd']))*8)):
                    ((hubnames[i])[2]).append('0')
            hubnames = str(hubnames)
            save('hubnames', hubnames)
            gv.hubnames = hubnames
            #end of new code
            ...
            save('snames', nstr)
            #start of new code need to remove 8 valves when expansion board number is decreased
            hubnames = data('hubnames')
            hubnames = ast.literal_eval(hubnames)
            for i in range(0, gv.sd['sh']):
                j = (((nbrd)*8)-1)
    ...
    def update_shcount(self, qdict):   
	"""Increase or decrease the number of sensor hubs displayed when number of sensor hubs is changed in options."""
        if int(qdict['osh']) > gv.sd['sh']: #Lengthen lists
            hubnames = data('hubnames')
            hubnames = ast.literal_eval(hubnames)
            hlength = len(hubnames)
            hubnames = str(hubnames)
            hubnames = hubnames[:-1]
            for i in range(0, (int(qdict['osh']))-(gv.sd['sh'])):
                hubnames += ",['Sensor Hub %d','MAC Address %d',[" % ((hlength + i + 1),(hlength + i + 1))
		for j in range(0, ((gv.sd['nbrd'])*8)):
                    hubnames += "'0'"
                    if(j<(((gv.sd['nbrd'])*8)-1)):
                        hubnames += ","
                    else:
            ...
            save('hubnames', hubnames)
            gv.hubnames = hubnames
        elif int(qdict['osh']) < gv.sd['sh']:
            hubnames = data('hubnames')
            hubnames = ast.literal_eval(hubnames)
            hlenth = len(hubnames)
            i = ((gv.sd['sh'])-1)
            while i > ((int(qdict['osh']))-1):
                hubnames.pop(i)
                i -= 1
            hubnames = str(hubnames)
            save('hubnames', hubnames)
...
class change_hubs:
    def GET(self):
        ...
        for i in range(0, gv.sd['sh']):
            hubnames += "["
            if qdict.has_key('sh'+str(i)):
                hubnames += "'" + qdict['sh'+str(i)] + "',"
            else:
                hubnames += "Sensor Hub "+str(i+1) +","
            if qdict.has_key('mac'+str(i)):
                hubnames += "'" + qdict['mac'+str(i)] + "',"#+ ",["
            else:
                hubnames += "'MAC Address "+str(i+1)
            if qdict.has_key('vac'+str(i)):
                valvestring = str(qdict['vac'+str(i)])
                valvestring = list(valvestring)
                valvestring = str(valvestring)
                hubnames += valvestring
            else:
                hubnames += "'0'"
            hubnames += "]"
            if i < ((gv.sd['sh'])-1):
                ...
                hubnames += "]"
        save('hubnames', hubnames.encode('ascii', 'backslashreplace'))
        jsave(gv.sd, 'sd')
        hubnames = ast.literal_eval(hubnames)
        hubnames = map(int,((hubnames[0])[2]))
        gv.hubnames = hubnames
        raise web.seeother('/')
...
class view_sensorhubs:
    def GET(self):
        checkLogin()
        gv.baseurl = baseurl()
        gv.cputemp = CPU_temperature()
        render = web.template.render('templates', globals={ 'gv': gv, 'str': str, 'eval': eval, 'data': data})
        return render.sensorhubs()
...
class run_now:
    ...
    def GET(self):
        verifyLogin()
        qdict = web.input()	#sets qdict equal to a web storage object that contains variables form the url (http://example.com/test?id=10 web.input() -> id=10)
        pid = int(qdict['pid'])
